...
# The sandbox backends use the Unix-only `resource` module for rlimits; on
# Windows the whole module cannot even be imported, so skip cleanly.
try:
    import resource  # noqa: F401
except ImportError:
    pytest.skip("sandbox rlimits require the Unix-only `resource` module", allow_module_level=True)
