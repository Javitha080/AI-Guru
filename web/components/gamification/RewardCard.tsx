...
export default function RewardCard({ level, title, currentXp, nextLevelXp, streak, recentBadges }: RewardCardProps) {
  ...
  return (
    <div className="bg-[var(--secondary)] rounded-xl p-5 border border-[var(--border)] w-full">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-orange-500/10 text-orange-500 font-bold border border-orange-500/20">
        ...
        <div>
          ...
          <div className="flex gap-2">
            {recentBadges.map((badge, idx) => (
              <div key={idx} className="w-8 h-8 rounded-full bg-[var(--background)] border border-[var(--border)] flex items-center justify-center text-[var(--foreground)] tooltip" title={badge}>
                <Award size={16} />
              </div>
            ))}
          </div>
