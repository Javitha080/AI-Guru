...
export default function QuizViewer({
  ...
  const handleAiJudge = useCallback(() => {
    ...
    const handle = startQuizJudge(
      ...
      },
      {
        ...
        onGrade: (grade) => {
          // Machine-readable verdict from the backend judge — append a compact
          // score line to the stored judgment so history shows real scores.
          setJudgments((prev) => {
            const current = prev[idx] ?? EMPTY_JUDGMENT;
            return {
              ...prev,
        ...
        onDone: () => {
          let finalText = "";
          let gradeInfo: QuizJudgeGrade | undefined;
          setJudgments((prev) => {
            const current = prev[idx] ?? EMPTY_JUDGMENT;
            finalText = current.text;
            gradeInfo = current.grade;
            return {
              ...prev,
          ...
          judgeHandlesRef.current.delete(idx);
          // Persist the AI judgment text (+ machine score) on the notebook
          // entry so it survives a page refresh. Best-effort — a failed write
          // just means the next reload won't have the judgment cached.
          const key = q ? getQuestionKey(q, idx) : "";
          const eId = key ? entryIds[key] : undefined;
          if (eId && finalText.trim().length > 0) {
            const payload = gradeInfo
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-[var(--glass-border)] surface-glass-elevated">
        ...
        <div className="flex flex-1 flex-wrap gap-1">
          {questions.map((question, questionIndex) => {
            ...
            const correctness: "correct" | "incorrect" | null =
                ...
                : null;
            const baseChip =
              "glass-capsule flex h-6 w-6 items-center justify-center text-[10px] font-semibold transition-all";
        ...
        {isChoice ? (
          <div className="space-y-1.5">
            {Object.entries(q.options!).map(([key, text]) => {
              ...
              let optionClass =
                "border-[var(--glass-border)] surface-glass-base text-[var(--foreground)] hover:border-[var(--primary)]/50";

              if (isSelected && !showFeedback) {
                optionClass =
                  "border-[var(--primary)] text-[var(--foreground)] ring-1 ring-[var(--primary)]/30 bg-[color-mix(in_srgb,var(--primary)_15%,var(--glass-1))]";
              ...
              return (
                <button
                  ...
                  className={`glass-capsule flex w-full items-start gap-2.5 px-4 py-3 text-left text-[13px] transition-all ${optionClass}`}
                >
                  <span
            ...
            const renderTFButton = (key: "true" | "false", label: string) => {
              ...
              return (
                <button
                  ...
                  disabled={ans.submitted}
                  onClick={() => updateAnswer({ selected: key })}
                  className={`glass-capsule flex flex-1 items-center justify-center gap-2 px-3 py-3 text-[14px] font-semibold transition-all ${cls}`}
                >
                  {label}
                </button>
              );
