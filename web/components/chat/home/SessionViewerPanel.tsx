...
function SessionViewerPanelInner(
  ...
  return (
    <div
      role="dialog"
      aria-hidden={!visible}
      className={`fixed right-0 top-0 z-[30] flex h-dvh flex-col border-l border-[var(--border)] surface-glass-elevated transition-transform ease-out max-md:!w-full md:max-w-[92vw] ${
    ...
    >
      ...
      />
      <div className="relative flex-1 overflow-hidden surface-glass-base m-2 rounded-xl">
        {activeTab?.kind === "file" ? (
...
function TabBar({
  ...
  const { t } = useTranslation();
  return (
    <div className="flex shrink-0 items-end gap-2 bg-transparent px-2 pt-2 pb-1">
      <div className="flex min-w-0 flex-1 items-center gap-2 overflow-x-auto">
        ...
        <button
          ...
          className={`inline-flex shrink-0 items-center gap-1.5 glass-capsule py-1.5 pl-2.5 pr-3 text-[11.5px] font-medium transition-colors ${
            homeActive
              ? "bg-[color-mix(in_srgb,var(--primary)_15%,var(--glass-1))] text-[var(--foreground)]"
              : "bg-transparent text-[var(--muted-foreground)] hover:bg-[var(--glass-0)] hover:text-[var(--foreground)]"
          }`}
          title={t("Activity")}
        >
        ...
        {tabs.map((tab) => {
          ...
          return (
            <div
              key={tab.id}
              className={`group inline-flex max-w-[180px] shrink-0 items-center glass-capsule text-[11.5px] font-medium transition-colors ${
                active
                  ? "bg-[color-mix(in_srgb,var(--primary)_15%,var(--glass-1))] text-[var(--foreground)]"
                  : "bg-transparent text-[var(--muted-foreground)] hover:bg-[var(--glass-0)] hover:text-[var(--foreground)]"
              }`}
