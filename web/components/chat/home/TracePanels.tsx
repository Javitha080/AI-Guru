...
function getTraceHeader(
  ...
  if (
    ...
    // top-level status row carries the verbose "AI Guru Reflecting…"
...
function LiveFoldRow({
  ...
  return (
    <div className="border border-[var(--glass-border)] bg-[color-mix(in_srgb,var(--primary)_5%,var(--glass-0))] rounded-xl px-3 py-2 mb-2">
      <TraceRowHeader
        open={open}
        expandable
        active={active}
            ...
            <>
              {chip ? (
                ...
                <div className="flex items-baseline gap-1.5">
                  ...
                  <span
                    className={`min-w-0 truncate glass-capsule text-[var(--foreground)] shadow-[0_0_10px_var(--glow-primary)] px-2 py-0.5 ${
...
export function CallTracePanel({
  ...
  return (
    <div className="mb-3 space-y-0.5 surface-glass-base rounded-2xl p-3">
 ...
 *  outside AI Guru that the turn is talking to. */
...
function detectStreamingMode(
  ...
  for (let idx = events.length - 1; idx >= 0; idx -= 1) {
    ...
    if (event.type === "content" && callKind === "llm_final_response") {
      ...
      // still open; keep the top-level title on "AI Guru Exploring…" until
      // the bus moves on.
      if (event.stage === "exploring") return "exploring";
...
export function StreamingStatus({
  ...
  const name = agentName?.trim() || "AI Guru";
  ...
  else if (mode === "responded") modeLabel = t("AI Guru responded.");
 ...
 * reasoning trace once AI Guru stops working and starts (or has finished)
 * its answer.
 *
 ...
 * ("AI Guru Exploring… · 8s", settling to "AI Guru responded. · 10s")
 * with the exploring trace nested directly beneath it.
 *
 * The trace is expanded by default while AI Guru is still reasoning /
