...
/** "AI Guru_Introduction.pdf" → "AI Guru Introduction" — the card title
  ...
  return (
    <>
      {/* Activity block pinned to the TOP: the status header
          ("AI Guru Exploring… · 8s" → "AI Guru responded. · 10s") with
          the exploring trace nested beneath it — expanded while AI Guru is
  ...
  return (
    <div key={`${msg.role}-${index}`} className="group flex justify-end">
      ...
      >
        ...
        {editing ? (
          <div className="w-[min(620px,75vw)] rounded-2xl border border-[var(--primary)]/40 surface-glass-elevated bg-[color-mix(in_srgb,var(--primary)_12%,var(--glass-1))] px-3 py-2.5 text-[14px] leading-relaxed text-[var(--foreground)] shadow-sm">
            <textarea
              autoFocus
              value={draft}
          ...
          <div
            ...
            className="rounded-2xl surface-glass-elevated bg-[color-mix(in_srgb,var(--primary)_12%,var(--glass-1))] px-4 py-2.5 text-[14px] leading-relaxed text-[var(--foreground)] shadow-sm"
          >
  ...
  return (
    <>
      {messageRows.map(({ msg, originalIndex, pairedUserMessage }) => {
        ...
        return (
          <div key={`${msg.role}-${i}`} className="w-full surface-glass-base rounded-2xl p-4">
            <InlineFileCardProvider
