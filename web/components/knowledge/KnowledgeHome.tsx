...
function EngineStatusBadge({ status }: { status: EngineStatus }) {
  ...
  if (status === "ready") {
    return (
      <span className="glass-capsule inline-flex items-center gap-1 text-[10px] font-medium text-emerald-700 dark:text-emerald-300">
  ...
  if (status === "needs_key") {
    return (
      <span className="glass-capsule inline-flex items-center text-[10px] font-medium text-amber-700 dark:text-amber-300">
        {t("Needs key")}
      </span>
    );
  }
  return (
    <span className="glass-capsule inline-flex items-center text-[10px] font-medium text-[var(--muted-foreground)]">
      {t("Not installed")}
    </span>
  );
}
      ...
      <div className="mx-auto max-w-4xl px-6 py-8">
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <button
            type="button"
            onClick={onCreate}
            className="inline-flex shrink-0 items-center gap-1.5 glass-btn-primary px-3.5 py-2 text-[12.5px] font-medium transition-opacity hover:opacity-90"
          >
            <Plus size={14} />
            ...
            {providers.map((p) => {
              ...
              return (
                <button
                  ...
                  className="group flex flex-col gap-2 surface-glass-elevated liquid-sheen rounded-2xl border border-[var(--glass-border)] p-3.5 text-left transition-colors hover:border-[var(--primary)]"
                >
          ...
          {kbs.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-[var(--border)] px-4 py-12 text-center">
              ...
              <button
                type="button"
                onClick={onCreate}
                className="mt-4 inline-flex items-center gap-1.5 glass-btn-primary px-3.5 py-2 text-[12.5px] font-medium transition-opacity hover:opacity-90"
              >
                <Plus size={14} />
                {t("New knowledge base")}
              </button>
              ...
              {filteredKbs.map((kb) => {
                ...
                return (
                  <button
                    ...
                    className="group flex flex-col gap-2 surface-glass-elevated liquid-sheen rounded-2xl border border-[var(--glass-border)] p-4 text-left transition-colors hover:border-[var(--primary)]"
                  >
