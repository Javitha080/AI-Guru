...
export interface QuizJudgeGrade {
...
export interface QuizJudgeHandlers {
  ...
  /** Machine-readable verdict frame emitted at the end of the stream (optional). */
  onGrade?: (grade: QuizJudgeGrade) => void;
}
...
export function startQuizJudge(
  ...
  ws.onmessage = (ev) => {
    ...
    if (frame.type === "grade") {
      const g = frame as unknown as { verdict?: string; score?: number };
      if (g.verdict && typeof g.score === "number") {
        handlers.onGrade?.({
          verdict: g.verdict as QuizJudgeGrade["verdict"],
          score: g.score,
        });
      }
      return;
