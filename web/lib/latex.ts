...
export function convertSequenceFenceToMermaid(source: string): string | null {
  ...
  const converted = normalizedLines.map((line) => {
    ...
    // Participants may contain spaces ("AI Guru") — match lazily up to the
    // arrow and greedily to the message colon, then quote spaced names for
    ...
    if (messageMatch) {
      ...
      return `  ${quote(from.trim())}${arrow}${quote(to.trim())}: ${message}`;
    }
