...
export default function ChatPage() {
  ...
  // How many times AI Guru may consult the selected agent this turn. Seeded
  ...
  const handleSend = useCallback(
    async (content: string) => {
      ...
      // (how many times AI Guru may ask it) so the subagent capability uses it.
  ...
  return (
    <QuizFollowupProvider>
      <GeogebraTabProvider>
        ...
        >
          <div className={`mx-auto flex w-full max-w-[960px] flex-wrap items-center justify-between gap-x-3 gap-y-1.5 px-6 pt-3 pb-0 ${!hasMessages ? "hidden" : ""}`}>
            <div className="group/title min-w-0 flex flex-1 items-center gap-2">
              {sessionTitleEditing ? (
                <input
                  ...
                  className="min-w-0 flex-1 rounded-xl border border-[var(--border)] bg-[var(--background)] px-3 py-1.5 font-display text-[15px] font-semibold tracking-[-0.01em] text-[var(--foreground)] shadow-sm outline-none transition focus:border-[var(--ring)] focus:ring-2 focus:ring-[var(--ring)]/20 disabled:opacity-60"
                  maxLength={100}
                />
              ) : (
                <button
                  ...
                  className="inline-flex min-w-0 max-w-full items-center gap-2 rounded-xl px-2 py-1 text-left font-display text-[15px] font-semibold tracking-[-0.01em] text-[var(--foreground)] transition hover:bg-[var(--muted)]/55 disabled:cursor-default disabled:hover:bg-transparent"
                >
