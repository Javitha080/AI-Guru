...
function normalizeDraft(payload: NetworkSettingsPayload): NetworkSettings {
  return {
    ...
    lan_access_enabled: Boolean(payload.settings.lan_access_enabled),
  };
}
    ...
    <div data-tour="tour-network">
      ...
      {!loading && payload && draft && (
        <>
          ...
          {/* LAN access (Parent Portal) */}
          <SettingSection
            title={t("LAN access")}
            description={t(
              "Let devices on your Wi-Fi open the Parent Portal directly, without a tunnel. Takes effect after restart.",
            )}
          >
            <div className="py-4 space-y-3">
              <label className="flex items-center gap-3 cursor-pointer select-none">
                <input
                  type="checkbox"
                  checked={Boolean(draft?.lan_access_enabled)}
                  onChange={(event) =>
                    setDraft((prev) =>
                      prev ? { ...prev, lan_access_enabled: event.target.checked } : prev,
                    )
                  }
                  className="h-4 w-4 accent-[var(--primary)]"
                />
                <span className="text-[13px] font-medium">
                  {t("Allow devices on my network to reach AI Guru (bind 0.0.0.0)")}
                </span>
              </label>
              {draft?.lan_access_enabled && (
                <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-[12px] leading-relaxed text-amber-700 dark:text-amber-300">
                  {t(
                    "The backend will listen on all network interfaces. The Parent Passcode still protects parent pages, but anyone on the LAN can reach the app — enable only on trusted home networks.",
                  )}
                </div>
              )}
            </div>
