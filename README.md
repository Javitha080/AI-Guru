...
<p align="center"><img src="assets/figs/logo/logo.png" alt="AI Guru logo" height="56" style="vertical-align: middle;">&nbsp;<img src="assets/figs/logo/banner.png" alt="AI Guru" height="48" style="vertical-align: middle;"></p>

# AI Guru — Local-First AI Tutoring & Study Supervision

> Formerly **DeepTutor**. The agent-native tutoring core is unchanged
> (`pip install deeptutor`); this distribution adds the local-first student
> monitoring, past-paper exam room, and passcode-protected parent portal.
> See [docs/AI-GURU-PARENT-ACCESS.md](docs/AI-GURU-PARENT-ACCESS.md) and the
> Exam Room (`/exam`) + Study Room (`/study-room`) in the web UI.
...
### 🎓 AI Guru — Intelligent Study Monitoring

AI Guru extends DeepTutor with a **local-first, privacy-focused study monitoring** platform. All computer vision runs 100% on-device — **zero camera frames or biometric data** ever leave your machine.

| Feature | Description |
|---------|-------------|
| **📹 Study Monitoring** | Local CV pipeline: face detection, liveness verification, pose/gaze tracking, engagement scoring, distraction analysis with study gesture whitelist |
| **⏱️ Study Sessions** | Pomodoro-style timer, pre-flight camera checks, real-time focus tracking, AI-generated session reports |
| **🏆 Gamification** | XP points with focus multipliers, daily streaks, 15 achievement badges, 50-level progression system |
| **👨‍👩‍👧 Parent Dashboard** | Secure pairing (`GURU-XXXX` codes), remote dashboard access via Cloudflare Tunnel/ngrok, opt-in live video |
| **🔒 Privacy-First** | Local SQLite database, 127.0.0.1 binding, AES-GCM encrypted backups, granular data deletion |
| **📡 Offline Resilient** | Continues monitoring + timer without internet, auto-fallback to Ollama, connectivity status indicator |

**Quick Start:**
```bash
deeptutor start        # Launch backend + frontend
# Navigate to Study Room in the sidebar to begin
```

See [`docs/AI-GURU-LOCAL-SETUP.md`](docs/AI-GURU-LOCAL-SETUP.md) for full setup, [`docs/AI-GURU-SECURITY.md`](docs/AI-GURU-SECURITY.md) for privacy architecture.
