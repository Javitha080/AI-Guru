...
def _wait_for_http(
    ...
    while time.monotonic() < deadline:
        ...
        if port and _port_accepts_connection(port):
            _log(_t("start.ready", name=name))
            return

        try:
            with urlrequest.urlopen(url, timeout=10):  # noqa: S310
                _log(_t("start.ready", name=name))
                return
...
def start(home: str | Path | None = None, *, dev: bool = False) -> None:
    ...
    # LAN access (Parent Portal "Allow devices on my Wi-Fi"): bind the
    # backend to all interfaces when enabled; the frontend stays loopback
    # because parents reach it through the backend-served UI / proxy.
    lan_access_enabled = False
    try:
        from deeptutor.services.config.runtime_settings import get_runtime_settings_service

        lan_access_enabled = bool(
            get_runtime_settings_service().load_system().get("lan_access_enabled")
        )
    except Exception:
        pass
    backend_host = "0.0.0.0" if lan_access_enabled else "127.0.0.1"
    ...
    try:
        ...
        while not shutdown_requested:
            ...
            if backend is not None and backend.process.poll() is not None:
                ...
                _log(_t("start.exited", name=backend.name, code=code))
                if recovery_counts["backend"] < max_recovery_attempts and not shutdown_requested:
                    recovery_counts["backend"] += 1
                    _log(
                        f"[Supervisor] Backend process exited unexpectedly (code {code}). "
                        f"Attempting recovery ({recovery_counts['backend']}/{max_recovery_attempts})..."
                    )
                    try:
                        _terminate(backend)
                        backend = _spawn(
                        ...
                        _log("[Supervisor] Backend process recovered successfully.")
                    except Exception as err:
                        _log(f"[Supervisor] Backend recovery failed: {err}")
                        exit_code = 1
                        shutdown_requested = True
                        break
                else:
            ...
            # Monitor frontend (if managed by us)
            if web is not None and web.process.poll() is not None:
                code = web.process.returncode
                _log(_t("start.exited", name=web.name, code=code))
                if recovery_counts["frontend"] < max_recovery_attempts and not shutdown_requested:
                    recovery_counts["frontend"] += 1
                    _log(
                        f"[Supervisor] Frontend process exited unexpectedly (code {code}). "
                        f"Attempting recovery ({recovery_counts['frontend']}/{max_recovery_attempts})..."
                    )
                    try:
                        _terminate(web)
                        web = _spawn(
                        ...
                        _log("[Supervisor] Frontend process recovered successfully.")
                    except Exception as err:
                        _log(f"[Supervisor] Frontend recovery failed: {err}")
                        exit_code = 1
                        shutdown_requested = True
                        break
                else:
