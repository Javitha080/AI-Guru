...
class StreakTracker:
    ...
    async def record_study_day(self, student_id: str) -> None:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            if last_date_str:
                ...
                if days_diff == 1:
                    ...
                    async with db.execute(
                        "SELECT streak_count FROM students WHERE id = ?", (student_id,)
                    ) as cursor:
                        row = await cursor.fetchone()
                    ...
                    if freezes >= days_diff - 1:
                        ...
                        await self._set_setting(
                            db, f"freezes_{student_id}", str(freezes - (days_diff - 1))
                        )
                        async with db.execute(
                            "SELECT streak_count FROM students WHERE id = ?", (student_id,)
                        ) as cursor:
            ...
            await db.execute(
                "UPDATE students SET streak_count = ? WHERE id = ?", (streak_count, student_id)
            )
            ...
            if last_date_str:
                ...
                if days_diff > 1:
                    ...
                    if freezes < days_diff - 1:
                        # Streak broken
                        await db.execute(
                            "UPDATE students SET streak_count = 0 WHERE id = ?", (student_id,)
                        )
                        await db.commit()
                        streak_count = 0
                    else:
    ...
    async def get_streak_info(self, student_id: str) -> Dict[str, Any]:
        ...
        return {"streak_count": streak, "last_study_date": last_date, "freezes_available": freezes}
