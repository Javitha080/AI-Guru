...
class BadgeEngine:
    ...
    async def _get_earned_badge_ids(self, student_id: str) -> Set[str]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute("SELECT reward_type FROM rewards WHERE student_id = ? AND reward_type LIKE 'badge_%'", (student_id,)) as cursor:
    ...
    async def check_and_award(self, student_id: str, session_id: str = None) -> None:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            total_xp = 0
            streak_count = 0
            async with db.execute("SELECT total_xp, streak_count FROM students WHERE id = ?", (student_id,)) as cursor:
            ...
            # Get session stats
            sessions_count = 0
            async with db.execute("SELECT COUNT(*) FROM study_sessions WHERE student_id = ?", (student_id,)) as cursor:
                row = await cursor.fetchone()
                if row:
                    sessions_count = row[0]

            # Simple logic for XP badges
            if total_xp >= 100 and 'xp_100' not in earned: to_award.add('xp_100')
            if total_xp >= 1000 and 'xp_1000' not in earned: to_award.add('xp_1000')
            if total_xp >= 10000 and 'xp_10000' not in earned: to_award.add('xp_10000')

            # Streak badges
            if streak_count >= 7 and 'streak_7' not in earned: to_award.add('streak_7')
            if streak_count >= 30 and 'streak_30' not in earned: to_award.add('streak_30')
            if streak_count >= 100 and 'streak_100' not in earned: to_award.add('streak_100')

            # Session counts
            if sessions_count >= 1 and 'first_session' not in earned: to_award.add('first_session')
            if sessions_count >= 10 and 'ten_sessions' not in earned: to_award.add('ten_sessions')
            if sessions_count >= 50 and 'fifty_sessions' not in earned: to_award.add('fifty_sessions')

            # Time based badges (need actual session logic, left simple for now)
            if session_id:
                async with db.execute("SELECT focus_score, actual_duration_seconds FROM study_sessions WHERE id = ?", (session_id,)) as cursor:
                    row = await cursor.fetchone()
                    if row:
                        focus, duration = row
                        if focus == 100 and 'focus_master' not in earned: to_award.add('focus_master')
                        if duration and duration >= 3600 and 'hour_warrior' not in earned: to_award.add('hour_warrior')
                        if duration and duration >= 10800 and 'marathon' not in earned: to_award.add('marathon')
