...
class XPEngine:
    ...
    async def award_session_xp(self, student_id: str, session_id: str, study_minutes: float, focus_score: float) -> int:
    ...
    async def get_student_xp(self, student_id: str) -> Dict[str, Any]:
        ...
        total_xp = 0
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute("SELECT total_xp FROM students WHERE id = ?", (student_id,)) as cursor:
    ...
    def get_level_title(self, level: int) -> str:
        ...
        elif level <= 20: return 'Scholar'
        elif level <= 30: return 'Expert'
        elif level <= 40: return 'Master'
