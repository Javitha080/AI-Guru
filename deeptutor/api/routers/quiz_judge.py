...
_JUDGE_SYSTEM_PROMPTS = {
    "zh": (
        ...
        "- 直接以学习者的作答为对象，不要泛泛而谈。\n"
        "- 全程使用中文。\n"
        "- 最后单独一行输出机器可读的 JSON 判定（除此之外不要输出其他 JSON）：\n"
        '  {"grade": {"verdict": "correct|partial|incorrect", "score": 0.0到1.0}}'
    ),
    "en": (
        ...
        "- Reply in English.\n"
        "- End with ONE final line containing machine-readable JSON (no other JSON):\n"
        '  {"grade": {"verdict": "correct|partial|incorrect", "score": <0.0-1.0>}}'
    ),
}


def _extract_grade_json(full_text: str) -> dict | None:
    """Pull the trailing machine-readable grade frame out of a judge response."""
        ...
        async for chunk in llm_stream(
            ...
            **stream_kwargs,
            ...
            if not await safe_send({"type": "text", "content": chunk}):
                break
        # Machine-readable verdict frame (consumed by the quiz UI for scoring).
        grade = _extract_grade_json("".join(full_text_parts))
        if grade:
            await safe_send({"type": "grade", **grade})
