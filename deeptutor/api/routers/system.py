...
async def get_system_health():
    """Return comprehensive health diagnostics for all AI Guru subsystems."""
    from deeptutor.services.hardware.health_checker import get_full_system_health
...
async def get_system_startup():
    """Get Windows auto-startup configuration status."""
    from deeptutor.services.platform.windows_startup import get_startup_status

    return await asyncio.to_thread(get_startup_status)
...
async def toggle_system_startup(payload: StartupToggleRequest):
    ...
    if payload.enabled:
        await asyncio.to_thread(enable_windows_startup, app_name=payload.app_name)
    else:
        await asyncio.to_thread(disable_windows_startup, app_name=payload.app_name)
    return await asyncio.to_thread(get_startup_status, app_name=payload.app_name)
