...
| Camera frames | RAM only (vision pipeline → WS → in-memory ring) | **Local-first on-device processing**: frames are processed on-device; unencrypted frames are never written to disk. Remote Telegram alerts include incident photo snapshots by default (configurable in parent settings). |
